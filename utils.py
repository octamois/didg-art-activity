...
def save():
    ...
    if dir==None: dir=''
    fname=os.path.join(dir,'data','DidgArt.dat')
    f=open(fname, 'w')
    load_save.save(f)
    f.close
...
def text_blit(screen,s,font, xxx_todo_changeme1, xxx_todo_changeme2,shadow=True):
...
def text_blit1(screen,s,font, xxx_todo_changeme3, xxx_todo_changeme4,shadow=True):
...
def display_number(n, xxx_todo_changeme9,font,colour=BLACK,bgd=None,outline_font=None):
...
def display_number2(screen,n, xxx_todo_changeme11,font,colour=BLACK):
    (cx,cy) = xxx_todo_changeme11
    if pygame.font:
        text=font.render(str(n),True,colour)
        centre_blit(screen,text,(cx,cy))

def display_number3(screen,n, xxx_todo_changeme12,font,colour=BLACK):
...
def top_left_to_centre(img, xxx_todo_changeme13):
    (x,y) = xxx_todo_changeme13
    cx=x+img.get_width()/2; cy=y+img.get_height()/2
    return (cx,cy)

def centre_to_top_left(img, xxx_todo_changeme14):
...
# requires a black image on a transparent bgd
def colour(img,colr):
   new_img=img.copy()
   new_img.fill(colr,None,pygame.BLEND_ADD)
   return new_img
